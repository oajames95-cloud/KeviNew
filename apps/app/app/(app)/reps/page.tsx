...
import { ChevronRight, Search, SlidersHorizontal, ArrowUpDown, MessageSquare } from "lucide-react"
...
function hasActiveCoachingItem(repId: string) {
  return mockCoachingInsights.some(
...
export default function RepsPage() {
  ...
  const filtered = useMemo(() => {
    let result = mockReps.filter((rep) => {
      const matchesSearch =
  ...
  const atRiskCount = mockReps.filter((r) => r.trend === "at-risk").length
  const driftingCount = mockReps.filter((r) => r.trend === "drifting").length
  ...
  return (
    <div className="flex flex-col flex-1 overflow-hidden">
      <AppHeader
        title="Reps"
        subtitle={`${needsAttentionCount} need attention · ${mockReps.length} total`}
        onMenuClick={toggle}
      />
